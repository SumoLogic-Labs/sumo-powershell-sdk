...
Describe "Start-SearchJob" {
  ...
  It "should startSearchJob with correct parameters with time range" {
    Mock startSearchJob -Verifiable {} -ModuleName "SumoLogic" -ParameterFilter {
    ...
    Assert-MockCalled startSearchJob -Exactly 1 -Scope It -ModuleName "SumoLogic"
  }

  It "should throw exception if time range not valid" {
    Mock startSearchJob -Verifiable {} -ModuleName "SumoLogic"
    {
    ...
    Assert-MockCalled startSearchJob -Exactly 0 -Scope It -ModuleName "SumoLogic"
  }
  ...
  It "should throw exception if job status not available" {
    ...
    Mock invokeSumoRestMethod -Verifiable {} -ModuleName "SumoLogic" -ParameterFilter { $function -eq "search/jobs/7" }
    {
      Start-SearchJob -Query "q" -Last "00:20:00"
    } | Should -Throw "Cannot get search job status"
    Assert-MockCalled startSearchJob -Exactly 1 -Scope It -ModuleName "SumoLogic"
    Assert-MockCalled invokeSumoRestMethod -Exactly 1 -Scope It -ModuleName "SumoLogic"
  }
  ...
  It "should throw exception if job cancelled" {
    ...
    Mock invokeSumoRestMethod -Verifiable {
      @{ "state" = "CANCELED" }
    } -ModuleName "SumoLogic" -ParameterFilter { $function -eq "search/jobs/7" }
    {
      Start-SearchJob -Query "q" -Last "00:20:00"
  ...
  It "should create search job and query result by id" {
    ...
    Assert-MockCalled startSearchJob -Exactly 1 -Scope It -ModuleName "SumoLogic"
    Assert-MockCalled invokeSumoRestMethod -Exactly 3 -Scope It -ModuleName "SumoLogic"
  }
}
